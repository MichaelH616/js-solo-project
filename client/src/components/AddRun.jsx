...
const AddRun = (props) => {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        if (!runNameError && !runTimeError && !runDistanceError) {
            axios.post('http://localhost:8000/api/runs', run)
                .then(res => {
                    setAllRuns([...allRuns, res.data])
                    navigate('/')
                })
                .catch(err => {
                    console.log(err);
    ...
    return (
        <div className="container-fluid vh-100" style={{ backgroundImage: `url(${background})`, backgroundSize: "cover" }}>
            ...
            <div className="container text-center fw-bold my-5">
                <div className="row">
                    <h4 className='my-2'>Add a new run!</h4>
                    <form action="" className="form col-4 offset-4 my-4" onSubmit={submitHandler}>
                        <div className="form-group">
                            ...
                            <input type="date" name="date" id="date" onChange={handleRunDate} value={run.date} max={new Date().toISOString().split('T')[0]}></input>
                            <br></br>
                            <button className='btn btn-success my-5'>Add a new run</button>
                        </div>
                    </form>
                </div>
            </div>
